...
# Parsing rules
def p_declaration(p):
    '''declaration : TYPE IDENTIFIER LBRACE para RBRACE LPARA inside RPARA '''
...
def p_inside_assign(p):
    '''inside : TYPE IDENTIFIER ASSIGN IDENTIFIER SEMICOLON'''
    p[0] = f'if {p[1]} {p[2]} {p[3]}:'

def p_inside_ret(p):
    '''inside : RET IDENTIFIER SEMICOLON'''
    p[0] = f'if {p[1]} {p[2]}:'
...
# Get user input
input_text = input("Enter a function definition: ")
# input_text ="int main(int x){return x;}"
